	...
	function BuildRoute(air_route_manager, air_build_manager, air_town_manager, /*city_from, city_to, */cargo_class)
	{
		...
		if (route != null) {
			if (typeof(route) == "instance") {
				...
				this.SwapCargoClass();
				AILog.Warning("Built " + AICargo.GetCargoLabel(Utils.GetCargoType(cargo_class)) + " air route between " + route.m_station_name_from + " and " + route.m_station_name_to + " in " + elapsed + " day" + (elapsed != 1 ? "s" : "") + ".");
				this.StopDateTimer();
				return true;
			}
...
//		AILog.Info("city_from = " + AITown.GetName(city_from) + " ; city_from_count = " + city_from_count + " ; max_town_station_from = " + max_town_station_from + " ; city_to = " + AITown.GetName(city_to) + " ; city_to_count = " + city_to_count + " ; max_town_station_to = " + max_town_station_to);
	...
	function ManageAircraftRoutes(air_town_manager)
	{
...
//			AILog.Info("Route " + i + " from " + AIBaseStation.GetName(AIStation.GetStationID(this.m_town_route_array[i].m_airport_from)) + " to " + AIBaseStation.GetName(AIStation.GetStationID(this.m_town_route_array[i].m_airport_to)));
...
//		AILog.Info("Managed " + this.m_town_route_array.len() + " air route" + (this.m_town_route_array.len() != 1 ? "s" : "") + " in " + management_ticks + " tick" + (management_ticks != 1 ? "s" : "") + ".");
...
//		local start_tick = AIController.GetTick();
		local num_vehs = this.GetAircraftCount();
