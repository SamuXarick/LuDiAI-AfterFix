...
//				AILog.Warning("Removed" + station_text + "road station tile at " + station_tile);
		...
		if (counter == 500) {
			::scheduled_removals.Road.rawset(this.m_station_from, 0);
...
//										AILog.Warning("case" + num_adjacent_road_tiles + "; Removed drive through station tile at " + tile);
								...
								if (counter == max_num_tries) {
									::scheduled_removals.Road.rawset(tile, tile_has_road ? 0 : 1);
//									AILog.Error("Failed to remove drive through station tile at " + tile + " - " + AIError.GetLastErrorString());
									continue;
								} else {
									continue;
...
//									AILog.Warning("case" + num_adjacent_road_tiles + "; Removed drive through station tile at " + tile);
							...
							if (counter == max_num_tries) {
								::scheduled_removals.Road.rawset(tile, 1);
//								AILog.Error("Failed to remove drive through station tile at " + tile + " - " + AIError.GetLastErrorString());
								continue;
							} else {
								continue;
		...
		if (depot_tile != null) {
			...
			if (counter == 1) {
				...
				if (counter == 500) {
					...
					if (counter == 500) {
						::scheduled_removals.Road.rawset(depot_tile, 0);
						return null;
					} else {
						return null;
					}
