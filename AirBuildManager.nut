...
//				AILog.Info(this.GetAirportTypeName(a) + " (monthly maintenance cost = " + AIAirport.GetMonthlyMaintenanceCost(a) + ")");
			}
...
//			this.SetRouteFinished();
		...
		if (infrastructure && num_vehicles == 0 && AIGroup.GetNumVehicles(AIGroup.GROUP_ALL, AIVehicle.VT_ROAD) == 0 && AIGroup.GetNumVehicles(AIGroup.GROUP_ALL, AIVehicle.VT_RAIL) == 0 && AIGroup.GetNumVehicles(AIGroup.GROUP_ALL, AIVehicle.VT_WATER) == 0) {
			if (this.m_airport_from != -1 && this.m_airport_from > 0) {
				...
				if (engine_id != null) {
					local count = Utils.GetEngineOptimalDaysInTransit(engine_id, this.m_cargo_type, DAYS_INTERVAL, true, this.m_airport_from, this.m_airport_type_from);
...
//					AILog.Info("large_available = false [2]");
			...
			local large_engine;
			if (large_available) {
				large_engine = Utils.GetBestEngineIncome(large_engine_list, this.m_cargo_type, DAYS_INTERVAL);
...
//					AILog.Info("large_available = false [4]");
			...
			if (large_available) {
				...
				local large_max_order_dist = Utils.GetMaximumOrderDistance(large_engine[0]);
				...
				foreach (near_city_pair in air_town_manager.m_near_city_pair_array[this.m_cargo_class]) {
					if (this.m_city_from == near_city_pair[0]) {
						if (!air_route_manager.TownRouteExists(this.m_city_from, near_city_pair[1], this.m_cargo_class)) {
							...
							if (!randomize_towns && !this.m_all_routes_built && air_route_manager.HasMaxStationCount(this.m_city_from, large_city_to, this.m_cargo_class)) {
...
//								AILog.Info("air_route_manager.HasMaxStationCount(" + AITown.GetName(this.m_city_from) + ", " + AITown.GetName(large_city_to) + ", " + this.m_cargo_class + ") == " + air_route_manager.HasMaxStationCount(this.m_city_from, large_city_to, this.m_cargo_class));
			...
			if (large_available) {
				...
				local fake = Utils.DistanceRealFake(town_tile, this.m_airport_from);
...
//					AILog.Info("small_available = false [2]");
			...
			local small_engine;
			if (small_available) {
				small_engine = Utils.GetBestEngineIncome(small_engine_list, this.m_cargo_type, DAYS_INTERVAL);
...
//					AILog.Info("small_available = false [4]");
			...
			if (small_available) {
				/* If we have the tile of the first airport, we don't want the second airport to be as close or as further */
				local small_max_order_dist = Utils.GetMaximumOrderDistance(small_engine[0]);
				...
				foreach (near_city_pair in air_town_manager.m_near_city_pair_array[this.m_cargo_class]) {
					if (this.m_city_from == near_city_pair[0]) {
						if (!air_route_manager.TownRouteExists(this.m_city_from, near_city_pair[1], this.m_cargo_class)) {
							...
							if (!randomize_towns && !this.m_all_routes_built && air_route_manager.HasMaxStationCount(this.m_city_from, small_city_to, this.m_cargo_class)) {
...
//					AILog.Info("heli_available = false [2]");
			...
			local heli_engine;
			if (heli_available) {
				heli_engine = Utils.GetBestEngineIncome(heli_engine_list, this.m_cargo_type, DAYS_INTERVAL);
...
//					AILog.Info("heli_available = false [4]");
			...
			if (heli_available) {
				/* If we have the tile of the first airport, we don't want the second airport to be as close or as further */
				local heli_max_order_dist = Utils.GetMaximumOrderDistance(heli_engine[0]);
				...
				foreach (near_city_pair in air_town_manager.m_near_city_pair_array[this.m_cargo_class]) {
					if (this.m_city_from == near_city_pair[0]) {
						if (!air_route_manager.TownRouteExists(this.m_city_from, near_city_pair[1], this.m_cargo_class)) {
							...
							if (!randomize_towns && !this.m_all_routes_built && air_route_manager.HasMaxStationCount(this.m_city_from, heli_city_to, this.m_cargo_class)) {
...
//				AILog.Info("!large_available && !small_available && !heli_available");
		...
		foreach (t, _ in town_list) {
			...
			foreach (a, _ in this.m_airport_types) {
				...
				AILog.Info("a:Checking " + AITown.GetName(t) + " for an airport of type " + this.GetAirportTypeName(a));
				...
				foreach (tile, _ in tile_list) {
					if (this.m_airport_from > 0) {
						...
						if (Utils.DistanceRealFake(tile, this.m_airport_from) >= fake_dist) {
							tile_list[tile] = null;
							continue;
						}
					}
					...
					if (!randomize_towns && (!this.m_best_routes_built || infrastructure) && cargo_production < 18) {
						tile_list[tile] = null;
						continue;
					} else {
...
//					AISign.BuildSign(tile, ("" + noise + " <= " + allowed_noise + ""));
					...
					if (spread_rad && AIGameSettings.GetValue("distant_join_stations")) {
						...
						adjacent_station_id = this.GetAdjacentNonAirportStationID(airport_rectangle, spread_rectangle);
					}
	...
	function UpdateAircraftLists()
	{
		...
		foreach (engine_id, _ in all_engines) {
			if (AIEngine.IsValidEngine(engine_id) && AIEngine.IsBuildable(engine_id) && AIEngine.CanRefitCargo(engine_id, this.m_cargo_type)) {
				local income = Utils.GetEngineOptimalDaysInTransit(engine_id, this.m_cargo_type, DAYS_INTERVAL, true, from_location, from_type);
				switch (AIEngine.GetPlaneType(engine_id)) {
					case AIAirport.PT_BIG_PLANE: {
						this.m_big_engine_list[engine_id] = income[0];
						break;
					}
					case AIAirport.PT_SMALL_PLANE: {
						this.m_small_engine_list[engine_id] = income[0];
						break;
					}
					case AIAirport.PT_HELICOPTER: {
						this.m_helicopter_list[engine_id] = income[0];
						break;
					}
				}
			}
