...
//			setRouteFinish();
		...
		if (m_fromType != AIAirport.AT_INVALID && m_toType != AIAirport.AT_INVALID) {
			...
			if (!(TestBuildAirport().TryBuild(m_airportFrom, m_fromType, m_fromStationID))) {
				AILog.Error("Although the testing told us we could build 2 airports, it still failed on the first airport at tile " + m_airportFrom + ".");
				setRouteFinish();
				return null;
			}

			if (!(TestBuildAirport().TryBuild(m_airportTo, m_toType, m_toStationID))) {
				AILog.Error("Although the testing told us we could build 2 airports, it still failed on the second airport at tile " + m_airportTo + ".");
...
//						AILog.Warning("m_airportTo; Removed airport at tile " + m_airportFrom);
				...
				if (counter == 500) {
					LuDiAIAfterFix().scheduledRemovals.AddItem(m_airportFrom, 0);
				...
				for (local i = 0; i < airTownManager.m_nearCityPairArray.len(); ++i) {
					if (m_cityFrom == airTownManager.m_cityFrom(airTownManager.m_nearCityPairArray[i]) && cargoClass == airTownManager.m_cargoClass(airTownManager.m_nearCityPairArray[i])) {
						if (!airRouteManager.townRouteExists(m_cityFrom, airTownManager.m_cityTo(airTownManager.m_nearCityPairArray[i]), cargoClass)) {
							...
							if (AIController.GetSetting("pick_mode") != 1 && all_routes_built && airRouteManager.hasMaxStationCount(m_cityFrom, large_cityTo, cargoClass)) {
				...
				for (local i = 0; i < airTownManager.m_nearCityPairArray.len(); ++i) {
					if (m_cityFrom == airTownManager.m_cityFrom(airTownManager.m_nearCityPairArray[i]) && cargoClass == airTownManager.m_cargoClass(airTownManager.m_nearCityPairArray[i])) {
						if (!airRouteManager.townRouteExists(m_cityFrom, airTownManager.m_cityTo(airTownManager.m_nearCityPairArray[i]), cargoClass)) {
							...
							if (AIController.GetSetting("pick_mode") != 1 && all_routes_built && airRouteManager.hasMaxStationCount(m_cityFrom, small_cityTo, cargoClass)) {
				...
				for (local i = 0; i < airTownManager.m_nearCityPairArray.len(); ++i) {
					if (m_cityFrom == airTownManager.m_cityFrom(airTownManager.m_nearCityPairArray[i]) && cargoClass == airTownManager.m_cargoClass(airTownManager.m_nearCityPairArray[i])) {
						if (!airRouteManager.townRouteExists(m_cityFrom, airTownManager.m_cityTo(airTownManager.m_nearCityPairArray[i]), cargoClass)) {
							...
							if (AIController.GetSetting("pick_mode") != 1 && all_routes_built && airRouteManager.hasMaxStationCount(m_cityFrom, heli_cityTo, cargoClass)) {
...
//				AILog.Info("!large_available && !small_available && !heli_available");
		...
		for (local t = town_list.Begin(); !town_list.IsEnd(); t = town_list.Next()) {
			...
			for (local a = m_airport_types.Begin(); !m_airport_types.IsEnd(); a = m_airport_types.Next()) {
				...
				AILog.Info("Checking " + AITown.GetName(t) + " for an airport of type " + WrightAI.GetAirportTypeName(a));
