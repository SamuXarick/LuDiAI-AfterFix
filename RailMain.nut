function LuDiAIAfterFix::BuildRailRoute()
{
	...
	if (unfinished || (this.rail_route_manager.GetTrainCount() < max(AIGameSettings.GetValue("max_trains") - 10, 10)) && Utils.ListHasValue(this.rail_route_manager.m_routes_built.all, false)) {
...
//			AILog.Info("estimated_costs = " + estimated_costs + "; engine_costs = " + engine_costs + "; wagon_costs = " + wagon_costs + ", rail_costs = " + rail_costs + ", clear_costs = " + clear_costs + ", foundation_costs = " + foundation_costs + ", station_costs = " + station_costs + ", depot_costs = " + depot_costs);
			...
			if (city_from == null) {
				city_from = this.rail_town_manager.GetUnusedCity(this.rail_route_manager.m_routes_built.best[cargo_class], cargo_class);
				if (city_from == null) {
					if (AIController.GetSetting("randomize_towns")) {
						this.rail_town_manager.m_used_cities_list[cargo_class].Clear();
					} else if (!this.rail_route_manager.m_routes_built.best[cargo_class]) {
						this.rail_route_manager.m_routes_built.best[cargo_class] = true;
						this.rail_town_manager.m_used_cities_list[cargo_class].Clear();
...
//						this.rail_town_manager.m_near_city_pair_array[cargo_class].clear();
						if (!this.rail_route_manager.m_routes_built.all[cargo_class]) {
							AILog.Warning("All " + AICargo.GetCargoLabel(cargo_type) + " rail routes have been used!");
						}
						this.rail_route_manager.m_routes_built.all[cargo_class] = true;
					}
				}
			}
...
//				AILog.Info("New city found: " + AITown.GetName(city_from));
				...
				this.rail_town_manager.FindNearCities(city_from, min_dist, max_dist, this.rail_route_manager.m_routes_built.best[cargo_class], cargo_class);
				...
				foreach (near_city_pair in this.rail_town_manager.m_near_city_pair_array[cargo_class]) {
					if (city_from == near_city_pair[0]) {
						if (!this.rail_route_manager.TownRouteExists(city_from, near_city_pair[1], cargo_class)) {
							...
							if (!AIController.GetSetting("randomize_towns") && !this.rail_route_manager.m_routes_built.all[cargo_class] && this.rail_route_manager.HasMaxStationCount(city_from, city_to, cargo_class)) {
...
//								AILog.Info("this.rail_route_manager.HasMaxStationCount(" + AITown.GetName(city_from) + ", " + AITown.GetName(city_to) + ", " + cargo_class + ") == " + this.rail_route_manager.HasMaxStationCount(city_from, city_to, cargo_class));
		...
		if (unfinished || city_from != null && city_to != null) {
			...
			return this.rail_route_manager.BuildRoute(this.rail_build_manager, city_from, city_to, cargo_class, best_rail_type);
		}
	}
	return true;
}
...
//			AILog.Info("all_station_tiles has " + all_station_tiles.Count() + " tiles");
			...
			foreach (tile, _ in all_station_tiles) {
				...
				if (found) {
//					AILog.Info("scheduled_removals[AITile.TRANSPORT_RAIL] has tile " + tile);
