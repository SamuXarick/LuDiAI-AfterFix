...
class RailRouteManager
{
	...
	function BuildRoute(rail_build_manager, city_from, city_to, cargo_class, rail_type)
	{
		if (this.m_sent_to_depot_group == null) {
			this.m_sent_to_depot_group = [];
			for (local i = 0; i <= 1; i++) {
				this.m_sent_to_depot_group.append(AIGroup.CreateGroup(AIVehicle.VT_RAIL, AIGroup.GROUP_INVALID));
				assert(AIGroup.IsValidGroup(this.m_sent_to_depot_group[i]));
			}
			assert(AIGroup.SetName(this.m_sent_to_depot_group[0], "0: Trains to sell"));
			assert(AIGroup.SetName(this.m_sent_to_depot_group[1], "1: Trains to renew"));
		}

		local route = rail_build_manager.BuildRailRoute(city_from, city_to, cargo_class, this.m_sent_to_depot_group, this.m_routes_built.best[cargo_class], rail_type);
		...
		if (route != null) {
			if (typeof(route) == "instance") {
				...
				this.SwapCargoClass();
				AILog.Warning("Built " + AICargo.GetCargoLabel(Utils.GetCargoType(cargo_class)) + " rail route between " + route.m_station_name_from + " and " + route.m_station_name_to + " in " + elapsed + " day" + (elapsed != 1 ? "s" : "") + ".");
				this.StopDateTimer();
				return true;
			}
			...
			AILog.Error("t:" + elapsed + " day" + (elapsed != 1 ? "s" : "") + " wasted!");
			return false;
		}
	}
...
//		AILog.Info("city_from = " + AITown.GetName(city_from) + " ; city_from_count = " + city_from_count + " ; max_town_station_from = " + max_town_station_from + " ; city_to = " + AITown.GetName(city_to) + " ; city_to_count = " + city_to_count + " ; max_town_station_to = " + max_town_station_to);
	...
	function ResetRailManagementVariables()
	{
		...
		if (this.m_last_management_managed < 0) this.m_last_management_managed = 7;
	}
	...
	function ManageTrainRoutes(rail_town_manager)
	{
...
//		AILog.Info("Managed " + this.m_town_route_array.len() + " rail route" + (this.m_town_route_array.len() != 1 ? "s" : "") + " in " + management_ticks + " tick" + (management_ticks != 1 ? "s" : "") + ".");
...
//		local start_tick = AIController.GetTick();
		local num_vehs = this.GetTrainCount();
...
//			AILog.Info("managing route " + i + ". UpdateEngineWagonPair");
			this.m_town_route_array[i].UpdateEngineWagonPair();
			if (this.InterruptRailManagement(cur_date)) return;
		}
		this.ResetRailManagementVariables();
...
//			AILog.Info("managing route " + i + ". RemoveIfUnserviced");
			...
			if (this.m_town_route_array[i].RemoveIfUnserviced()) {
				...
				rail_town_manager.ResetCityPair(city_from, city_to, cargo_class, true);
			}
			if (this.InterruptRailManagement(cur_date)) return;
		}
		this.ResetRailManagementVariables();
